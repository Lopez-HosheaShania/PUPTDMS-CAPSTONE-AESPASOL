...
@section('content')
    ...
    @php
        ...
        $appointmentsRouteName =
            $routePrefix === 'dentist' ? 'dentist.dentist.appointments' : 'admin.admin.appointments';
        ...
        $scheduledRecordsCount =
            $scheduledCount ??
            $recordItems
                ->filter(fn($record) => strtolower(trim($record->status ?? 'scheduled')) === 'scheduled')
                ->count();
        ...
        $recordPerPage =
            $recordsSource instanceof \Illuminate\Pagination\AbstractPaginator ? $recordsSource->perPage() : 10;
                ...
                <section class="table-card dental-records-main-card">
                        ...
                        <div class="table-toolbar">
                            <div class="table-toolbar-search">
                                <div class="voice-search-row">
                                    <x-search-bar id="dentalRecordSearch" placeholder="Search patient name..."
                                        type="search" clear-label="Clear dental record search" class="flex-1" />
                            ...
                            <div class="table-toolbar-actions">
                                <button id="dentalRecordFilterBtn" type="button" class="global-filter-btn"
                                    aria-pressed="false" onclick="openDentalRecordFilters()" data-tooltip="Filter"
                    ...
                    @if ($recordsSource instanceof \Illuminate\Pagination\AbstractPaginator)
                        <x-pagination-bar id="dentalRecordsPagebarTop" info-id="dentalRecordsPageInfoTop"
                            ...
                            :page-size-value="$recordPerPage" label="records" :total="$recordsSource->total()" :from="$recordsSource->firstItem() ?? 0" :to="$recordsSource->lastItem() ?? 0" />
                    @endif
                                ...
                                <table class="data-table">
                                    ...
                                    <tbody id="dentalRecordsTableBody">
                                        @foreach ($recordsSource as $record)
                                            @php
                                                $rawStatus = strtolower(trim($record->status ?? 'scheduled'));

                                                $normalizedStatus =
                                                    $rawStatus === 'completed' ? 'completed' : 'scheduled';
                                                ...
                                                $dentistName = trim((string) $dentistName);

                                                if ($dentistName === '' || in_array($dentistName, ['—', '-'], true)) {
                                                    $dentistName = 'Unassigned';
                                                }

                                                $procedure = trim((string) ($record->procedure ?? ''));

                                                if ($procedure === '' || in_array($procedure, ['—', '-'], true)) {
                                                ...
                                                $dateText = $recordDate ? $recordDate->format('M d, Y') : 'Not set';
                                            ...
                                            <tr class="dental-record-row dental-record-item"
                                                ...
                                                data-dentist="{{ strtolower($dentistName) }}"
                                                data-status="{{ $normalizedStatus }}" data-date="{{ $dateIso }}"
                                                @if (!empty($record->appointment_id)) onclick="openRecordPanel({{ $record->appointment_id }})" @endif>
                            ...
                            <div id="dentalRecordGridView" class="table-record-grid dental-record-grid-view" hidden>
                                @foreach ($recordsSource as $record)
                                    @php
                                        ...
                                        $normalizedStatus = $rawStatus === 'completed' ? 'completed' : 'scheduled';

                                        $statusClass =
                                            $normalizedStatus === 'completed' ? 'status-completed' : 'status-upcoming';
                ...
                <aside class="dental-records-side-column space-y-5">
                    <section class="table-card rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
                            <div
                                class="dental-req-quick-actions-icon w-10 h-10 rounded-2xl bg-red-50 text-[#8B0000] 
    ...
    <x-filter-drawer id="filterModal" title="Filter Records" close-id="dentalRecordFilterCloseBtn"
        ...
        clear-callback="clearDentalRecordFilterDraft()" clear-label="Clear Filters"
        cancel-id="dentalRecordFilterCancelBtn" cancel-callback="closeDentalRecordFilters()" cancel-label="Cancel"
        apply-id="dentalRecordFilterApplyBtn" apply-callback="applyDentalRecordFilters()" apply-label="Show Results"
            ...
            <div class="filter-chip-row">
                <label class="choice-chip">
                    <input type="radio" name="dental_record_sort" value="newest"
                        class="filter-input radio-red chip-radio" data-record-filter="sort">
...
@section('scripts')
    <script>
        ...
        async function updateDentalRecordsPagination(pagination) {
            if (!pagination) {
                return;
            }
            ...
            if (typeof window.renderGlobalPagination !== 'function') {
                try {
                    await window.loadPaginationBarModule?.();
                } catch (error) {
                    console.error(
                        'Unable to initialize dental records pagination.',
                        error
                    );
            ...
            if (typeof window.renderGlobalPagination !== 'function') {
                console.error(
                    'Global pagination renderer is unavailable.'
                );
        ...
        function updateDentalRecordFilterDraftUi() {
            document
                ...
                .forEach(input => {
                    ...
                    if (key === 'sort') {
                        expected = ['newest', 'oldest'].includes(dentalRecordFilterDraft.sort) ?
                            dentalRecordFilterDraft.sort :
                            '';
                    }

                    if (key === 'name_sort') {
                        expected = ['name_asc', 'name_desc'].includes(dentalRecordFilterDraft.sort) ?
                            dentalRecordFilterDraft.sort :
                            '';
                    }
        ...
        function renderDentalRecordActiveFilters() {
            ...
            if (dentalRecordFilterDraft.datePreset !== 'all' || dentalRecordFilterDraft.dateFrom || dentalRecordFilterDraft
                .dateTo) {
                const dateLabels = {
                    today: 'Today',
                    7: 'Last 7 Days',
        ...
        function renderDentalRecordsEmptyState() {
            ...
            if (query) {
                window.EmptyState?.renderSearch({
                    ...
                    input: '#dentalRecordSearch',

                    query,

                    message: 'Try another patient name, procedure, dentist, or status.',
                });
        ...
        window.changeDentalRecordsPageSize =
            function(value) {
                ...
                dentalRecordsPerPage = [10, 20, 50, 100]
                    .includes(size) ?
                    size :
                    10;
