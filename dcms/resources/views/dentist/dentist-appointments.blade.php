...
<main id="mainContent"
    class="admin-page-shell admin-appointments-page dentist-page-shell dentist-appointments-page page-enter mode-list">
    <div class="w-full">
    ...
    <section id="upcomingSection" class="pb-16">
      ...
      updateUrl: '{{ route('dentist.dentist.appointments.reschedule.update', $appt->id) }}'
                ...
                <div class="mobile-appointments-list">
                    ...
                    $pastStatusClass = $isCancelledPast ? 'status-cancelled' : 'status-completed';
                    $recordDuration =
                    $appt->duration ?? ($appt->procedure_duration ?? ($appt->treatment_duration ?? ''));
                    $recordRemarks = $appt->remarks ?? ($appt->treatment_notes ?? ($appt->notes ?? ''));
                    $recordOral = $appt->oral_examination ?? ($appt->oral ?? '');
                    $recordDiagnosis = $appt->diagnosis ?? '';
                    $recordPrescription = $appt->prescription ?? '';
        ...
        <div id="appointmentStatusEmptyUpcoming" class="empty-state empty-state-controlled">
            ...
            <button type="button" onclick="resetAppointmentFilters()" class="appointment-panel-empty-clear hidden">
                <i class="fa-solid fa-rotate-left"></i>
                Clear filters
            </button>
        </div>
    ...
    <section id="pastSection" class="pb-16">
        ...
        trigger.addEventListener('click', function (event) {
            ...
            document.querySelectorAll('.appointment-status-dropdown.open').forEach(item => {
                if (item !== dropdown) {
                    ...
                    item.querySelector('.appointment-status-trigger')?.setAttribute('aria-expanded', 'false');
                }
            });
