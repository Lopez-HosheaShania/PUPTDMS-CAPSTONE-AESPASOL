                        ...
                        <div id="docreqStatusSelect"
                            ...
                            <button type="button" class="docreq-sort-trigger" id="docreqStatusDropdownBtn"
                                ...
                                <span class="docreq-sort-trigger-right">
                                    <span id="statusDropdownCount" class="docreq-sort-count">0</span>
                            ...
                            <div class="docreq-sort-panel" id="docreqStatusMenu" role="listbox">
                                <div class="docreq-sort-grid">
                                    <button type="button" class="docreq-sort-option is-active status-all"
                                        ...
                                        data-count="0" onclick="selectStatusFilter('all')">
                                        <span class="docreq-option-icon status-all">
                                            <i class="fa-solid fa-file-medical"></i>
                                        </span>
                                        <span class="docreq-option-label">All Requests</span>
                                        <span class="docreq-sort-option-count docreq-option-count" id="statAll">0</span>
                                    </button>

                                    <button type="button" class="docreq-sort-option status-pending" data-value="pending"
                                        data-label="Pending" data-icon="fa-clock-rotate-left" data-count="0"
                                        ...
                                        <span class="docreq-sort-option-count docreq-option-count"
...
<script>
    ...
    function renderList() {
        ...
        const lastPage = Math.max(1, Math.ceil(total / docreqPerPage));
        if (currentPage > lastPage) currentPage = lastPage;

        const start = (currentPage - 1) * docreqPerPage;
        const page = filtered.slice(start, start + docreqPerPage);

        const pagination = {
            total,
            from: total ? start + 1 : 0,
            to: total ? Math.min(start + docreqPerPage, total) : 0,
            current_page: currentPage,
            last_page: lastPage,
            per_page: docreqPerPage
        };

        renderDocreqPagebar(pagination);
...
`;
    ...
    function selectDocreqPerPage(value) {
        docreqPerPage = Number(value || 10);
        currentPage = 1;

        updateDocreqPerPageUI(docreqPerPage);
    ...
    function renderDocreqPagebar(p) {
        ...
        updateDocreqPerPageUI(p.per_page || docreqPerPage);
    ...
    function docreqGoPage(page) {
        ...
        document.getElementById('requestListContainer')?.scrollIntoView({
            behavior: 'smooth',
            block: 'start'
        });
    }
    ...
    function showDocreqError(message) {
        if (window.showToast) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.getElementById('approveConfirmBtn').addEventListener('click', async () => {
            ...
            if (!id) {
                showDocreqError('No document request was selected.');
                return;
            }
            ...
            try {
                ...
                if (!res.ok) {
                    const message = await readErrorMessage(res,
                        'Approval failed. Please refresh the page and try again.');
        ...
        document.getElementById('rejectConfirmBtn').addEventListener('click', async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    const message = await readErrorMessage(res,
                        'Rejection failed. Please refresh the page and try again.');
                    throw new Error(message);
                }
