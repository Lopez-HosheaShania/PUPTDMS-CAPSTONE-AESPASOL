...
@section('content')
    ...
    @php
        ...
        $countByStatus = function ($status) use ($docRequestsCollection, $normalizeStatus) {
            return $docRequestsCollection
                ->filter(function ($req) use ($status, $normalizeStatus) {
                            ...
                            <div class="docreq-search-wrap flex-1 md:flex-none flex items-center gap-2">
                                <div class="search-wrap global-search flex-1 md:w-64" data-search-wrapper>
                                    ...
                                    <input id="searchInput" type="text"
                                        placeholder="Search name, ID, reference, document…" data-search-input
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div class="filter-date-input-wrap">
                            <input type="text" id="fDateFrom" class="js-flatpickr-date-range-from"
                                placeholder="Start date" readonly autocomplete="off" />
                            <i class="fa-regular fa-calendar"></i>
                        </div>

                        <div class="filter-date-input-wrap">
                            <input type="text" id="fDateTo" class="js-flatpickr-date-range-to"
...
`;
    ...
    try {
        ...
        if (!res.ok) {
            ...
            alert(data.message || `Approval failed. Status: ${res.status}`);
            return;
        }
        ...
        alert('Approval failed because of a network or JavaScript error. Check the console.');
    } finally {
        btn.disabled = false;
    }
});
    ...
    try {
        ...
        if (!res.ok) {
            ...
            alert(data.message || `Rejection failed. Status: ${res.status}`);
            return;
        }
        ...
        alert('Rejection failed because of a network or JavaScript error. Check the console.');
    } finally {
        btn.disabled = false;
    }
});
