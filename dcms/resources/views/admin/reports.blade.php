...
<main id="mainContent" class="admin-page-shell page-enter mode-list">
    <div class="w-full">
        ...
        $inventoryLowStockCount = (int) ($inventory['low_stock_count'] ?? $inventoryItems->filter(function ($item) {
        ...
        return $minLevel > 0 && $inStock < $minLevel; })->count());
            ...
            $inventoryCriticalCount = $inventoryItems->filter(function ($item) {
            $inStock = (int) ($item['in_stock'] ?? 0);
            $minLevel = (int) ($item['min_level'] ?? 0);
                        ...
                        <div class="report-inventory-table-wrap report-movement-table-wrap">
                            <table class="inv-table report-movement-table">
                                ...
                                <tbody>
                                    ...
                                    $inStock = (int) ($item['in_stock'] ?? 0);
                                    $minLevel = (int) ($item['min_level'] ?? 0);
                                    $stockIn = (int) ($item['stock_in'] ?? ($item['restocked'] ?? ($item['received'] ??
                                    0)));
                                    $openingStock = (int) ($item['opening_stock'] ?? max(0, $inStock + $used -
                                    $stockIn));
                                    $averageDailyUsage = $used > 0 ? $used / $inventoryDaysElapsed : 0;
                                    $daysRemaining = $averageDailyUsage > 0 ? (int) floor($inStock / $averageDailyUsage)
                                    : null;
                                    $targetStock = max($minLevel * 2, $used);
                                        ...
                                        elseif ($minLevel> 0 && $inStock < ($minLevel * 0.5)) {
                                            ...
                                            (($minLevel> 0 && $inStock < $minLevel) || (! is_null($daysRemaining) &&
                                                ...
                                                <td class="forecast-days" data-label="Days left">
                                                    {{ is_null($daysRemaining) ? 'No usage yet' :
                                                    number_format($daysRemaining) . ' ' . Str::plural('day',
                                                    $daysRemaining) }}
                                                </td>
                                                <td class="forecast-num forecast-reorder"
                                                    data-label="Suggested reorder">{{ number_format($suggestedReorder)
                                                    }}</td>
                    ...
                    <div class="chart-grid">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="chart-legend">
                                    @foreach ($treatments['breakdown'] as $i => $proc)
                                    @php $colors = ['#378ADD','#1D9E75','#D85A30','#BA7517','#7F77DD','#9ca3af'];
                                    @endphp
                                    <span>
